...
1) Parameters - A hexadecimal number starting with '#' followed by either 6
                digits or 3 digits or a preset color name that references a 
                6 digit hex number.
2) Returns - Return a map with r, g, and b color values converted from the input
...
function parseHTMLColor(color) {
  let digits = [];
  // Convert a 6 digit hex number to the equivalent red, green, and blue (rgb)
  // color values and return as a object (map)
  const rgb = (hex) => {
  ...
  // Check if given color is a hex number
  if (color.includes("#")) {
    // Check if given hex number is 3 digits
    if (color.length === 4) {
      digits = color.split("");

      // Replace each hex digit by concatenating with itself making a
      // 6 digit hex number
      for (let i = 0; i < digits.length; i++) {
        if (digits[i] !== "#") digits[i] += digits[i];
      }
    ...
    // Color is a preset color name
    const preset = color.toLowerCase(); // ensure lowercase
    return rgb(PRESET_COLORS[preset]);
  }
}
