...
2) Return - An array with two numbers: the weights of team 1 and weights of team
            2.
...
function rowWeights(array) {
  // Filter weights for team 1 and find total weight
  const team1 = array
    .filter((item, index) => index % 2 === 0 || index === 0) // even indices & 0
    .reduce((acc, val) => acc + val, 0);
  // Do the same for team 2
  const team2 = array
    .filter((item, index) => index % 2 !== 0) // odd indices
    .reduce((acc, val) => acc + val, 0);
